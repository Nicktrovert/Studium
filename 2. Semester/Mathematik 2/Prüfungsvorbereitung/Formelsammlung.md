...
----------------------------------------------------------------------***ANALYSIS:***----------------------------------------------------------------------
**Länge:** $|v| = \sqrt{x^2+y^2}$ ------------------ **Skalarprodukt:**  $\overrightarrow{a}\cdot\overrightarrow{b}=a_x\cdot b_x+a_y\cdot b_y$ ----------------------**Winkel zw. 2 Vektoren:** $\cos{\alpha} = \frac{\overrightarrow{a}\cdot\overrightarrow{b}}{||\overrightarrow{a}||\cdot||\overrightarrow{b}||}$------------------**Orthogonal** <=> $\overrightarrow{a}\cdot\overrightarrow{b}=0$ -----------------**Gerade:** $\overrightarrow{x}=\overrightarrow{p}+t\cdot\overrightarrow{r}$ ----------------------**Parralel:** $r_1=\lambda\cdot r_2$ -----------------------------------**Vektor von A nach B:** $\overrightarrow{AB}=\pmatrix{x_{B}-x_{A}\\ y_{B}-y_{A}}$ ---------- **Abstand zweier Punkte:** $d(A,B)=\sqrt{(x_2 - x_1)^2+(y_2-y_1)²}$ 
...
**Abstand Punkt-Gerade:** Geg. $g: \overrightarrow{x}=\overrightarrow{p}+t\cdot\overrightarrow{r}$ , Punkt $Q$ ----- Schritt 1. Punkt auf Gerade $S(t) = \overrightarrow{p}+t\cdot\overrightarrow{r}$  -----
...
**Lineare Kongruenz:** $a\cdot x\equiv b \mod m$ ------- Gibt es lösung: $ggT(a,m) = 1$? ------ Ja: Inverse berechnen -> beide Seiten mit Inverser multiplizieren -> $x=a^{-1}\cdot b \mod m$ 
...
**ISBN-10:** $10\cdot a_1+9\cdot a_2+\dots+2\cdot a_9+p\equiv 0 \pmod{11}$ ---- Prüfziffer = p -> Ziffern mult. & summieren. -> $sum\cdot p \equiv 0 \pmod{11}$
...
**Matching:** kein Knoten doppelt ----- **Vollständiges Matching:** alle Knoten links einem Knoten rechts zugeordnet
